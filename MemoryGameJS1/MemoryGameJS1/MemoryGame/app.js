...
/* Här deklareras och tilldelas två variabler i varsina objekt. 
Objekten innehåller egenskaperna namn samt startpoäng för spelarana.*/
...
// Här tilldelas en varabel en array som sedan används för att kunna alternera spelare.
...
// När någon av spelarna trycker på knappen endGameBtn, döljs spelplanen och menyn visas igen.
...
function endGame() {
  ...
  // Här deklareras och tilldelas en variabel.
...
// När man klickar på återställ spel-knappen kallas funktionerna som gör att poängen och spelplanen återställs.
...
/* En funktion som skapar memorykorten: HTML-elementen div respektive img skapas och sparas i variabler.
Därefter adderas klasser till div-variabeln, och image-elementen tilldelas en src. 
Sedan läggs divarna läggs till i variabeln cardContainer och image-elementen läggs till i div-variabeln,
med metoden append. Därefter läggs allt in i den tomma listan cardArray. Image-elementen döljs sedan genom opacity 0. */
...
// Vi skapar en tom array till vilken vi sen ska pusha på vår gameObjectArray med både frontface-namn samt path.
let gameCardsArray = [];
...
// Vi använder den blandade listan för att rita ut korten på spelplanen.
createMemoryCards(randomizedArray);
...
/*Denna funktion testar om två kort matchar varandra.
src ska vara identisk men inte id, detta för att man inte ska kunna klicka på samma kort två gånger.
Därefter får spelaren som fått ett par, poäng.
Sedan kallas funktion som tar bort korrekta kort .
Om kortens src ej är identisk kallas funktion som gör att kortens opacity får 0 som värde, utefter angiven tidsram. 
Då blir det nästa spelares tur. */
...
Här kallas även en funktion som visar spelets vinnare, dvs. om en av spelarna når upp till 2 poäng.*/
...
Vi lägger in de valda kortens src och id i en listan choosenCards.
De valda kortens bilder, images blir synliga med opacity 1.
Vi lägger till classen som gör anmiationen möjlig.*/
...
/* Listan choosenCards tar enbart emot 2 kort i en array som då kontrollerar om de matchar,
 sedan töms denna array igen.*/
...
/* Funktion som adderar poäng till den spelare som fått par. 
Poängen läggs till i respektive spelares <p>-tagg i html-koden.*/
function getPoints() {
  if (switchPlayer === 1) {
...
/* Funktion som utser en vinnare om en spelare fått 2 poäng. 
En alert med text kommer upp. 
Detta sker 0.7 sek efter spelaren fått 2 poäng. 
Tidsangivelsen möjliggjordes med funktionen SetTimeout för att kort skulle visas 
och poäng skulle ges innan alert visades.*/ 
...
// En variabel tilldelas startvärdet 1.
let switchPlayer = 1;

// En funktion som bestämmer vilken spelares tur det är. Funktionen kallas i funktionen doesCardsMatch, rad 265.
